...
def test_evaluate_all_packages_returns_dataframe():
    ...
    assert len(df) == 8 * 2  # 8 packages × 2 ε values
...
def test_package_rankings_pivot():
    ...
    assert rankings.shape == (3, 8)  # 3 epsilons × 8 packages
    # Each row should produce ranks in [1, 8] (allowing ties via method="min")
    for _, row in rankings.iterrows():
        assert row.min() == 1.0
        assert row.max() <= 8.0
        # No more than 8 distinct ranks
